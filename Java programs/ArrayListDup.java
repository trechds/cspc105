...
public class ArrayListDup {
	...
	public static void main(String[] args) {
		...
		do {
			...
			ArrayList<Integer> dupsValue = dups(numbers);
			System.out.println("The following numbers were duplicated in the ArrayList: " + dupsValue);
	...
	public static ArrayList<Integer>dups(ArrayList<Integer> array){
        ArrayList<Integer> duplicateValue = new ArrayList<Integer>();
        int value = 0;
        for (int i = 0; i < array.size(); i++) {
            value = array.get(i);
            if (array.subList(i + 1, array.size()).contains(value)) {
                if(!duplicateValue.contains(array.get(i))) {
                    for (int j = 0; j < array.size()-1; j++) {
                        if ((array.get(j).equals(array.get(i)))) {
                            duplicateValue.add(array.get(i));
                            break;
                        }
                    }
                }
