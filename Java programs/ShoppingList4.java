...
public class ShoppingList4 {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case "5":
				...
				break;
			case "6":
				openList(sIn, shoppingList, "ShoppingList.txt");
				break;
	...
	private static void openList(Scanner sIn, ArrayList<String> shoppingList, String FILENAME) {
		...
		if (shoppingList.size() != 0) {
			boolean valid = false;
			do {
				System.out.println("Are you sure you wish to overwrite your current shopping list with the 'Shopping List.txt' file? (Y/N)");
		...
		if (overwrite) {
			File shoppingList2 = new File (FILENAME);
			if (!shoppingList2.exists()) {
				System.out.println("File Error: '" + FILENAME + "'  not found! Please save the list before trying to open it.");
				return;
			}
			
			Scanner fileIn = null;
			try {
				fileIn = new Scanner ("ShoppingList.txt");
				fileIn.nextLine();
				
				while (fileIn.hasNextLine()) {
					String shoppingItems = fileIn.nextLine();
					shoppingList.add(shoppingItems);
				}
